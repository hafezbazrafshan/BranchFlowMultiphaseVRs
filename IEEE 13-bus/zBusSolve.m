...
Network.Solution.v =performZBus( Network.Solution.w,Network.Branch, Network.Bus,...
    Network.Y,Network.YNS,40,Network.vS,Network.VMIN,Network.VMAX); % we don't use the bounds
...
Av=Network.Branch.ClosedDeltaAvs{rr};
     ...
     Network.Solution.v3PhaseRegs(r,:)=(inv(Av)*Network.Solution.v3Phase(n,:).').'; % three phases
     Network.Solution.w3PhaseRegs(r,:)=(inv(Av)*Network.Solution.w3Phase(n,:).').';
end
