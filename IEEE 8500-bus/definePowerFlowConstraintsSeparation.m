...
for n=1:NBuses
    ...
    for Phi=1:length(PhaseSet)
     ...
     for jj=1:length(Bus.ToNeighborsRegulatorBranchNumbers{n})
          ...
          elseif length(Branch.Phases{l})==2
              if ismember(Phi,Branch.Phases{l})
              rr=find(Branch.Wye2PhiBranchNumbers==l);
              PhiIdx=find(Branch.Phases{l}==Phi);
              ToNeighborsRegSum=ToNeighborsRegSum+SnnPrime2Phi(PhiIdx,rr);
               % power flow equation for n'
              ...
              SnmPrime2Phi(PhiIdx,rr)==trace( conj(YNMnTilde.')*EnTilde*WnnPrimeTilde)-...
                 trace( conj(YNMmTilde.')*EnTilde*WnmPrimeTilde);
              end
          else
               if ismember(Phi,Branch.Phases{l})
              rr=find(Branch.Wye1PhiBranchNumbers==l);
         ToNeighborsRegSum=ToNeighborsRegSum+SnnPrime1Phi(1,rr);
         % power flow equation for n'
              ...
              SnmPrime1Phi(1,rr)==trace( conj(YNMnTilde.')*EnTilde*WnnPrimeTilde)-...
                 trace( conj(YNMmTilde.')*EnTilde*WnmPrimeTilde);
