...
for n=1:NBuses % project onto three-phase space
    PhaseSet=Bus.Phases{n};
     ...
     if length(PhaseSet)==3
            ...
            SnTilde(PhaseSet,PhaseSet)=Bus.SLoad{n};
 ...
 ZNMTilde(Branch.Phases{l},Branch.Phases{l})=Av*Branch.Admittance{l}.ZNM*Av.';
        ...
        for jj=1:length(Bus.FromNeighborsRegulatorBranchNumbers{n})
           ...
           ZNMTilde=zeros(3); 
            ...
            if length(Branch.Phases{l})==3
                ll=find(Branch.ThreePhaseBranchNumbers==l); 
                SnmTilde(Branch.Phases{l},Branch.Phases{l})=Inm3Phi(:,:,ll);
                 InmTilde(Branch.Phases{l},Branch.Phases{l})=Inm3Phi(:,:,ll);
                 rr=find(Branch.Regs3PhiBranchNumbers==l); 
                if strcmp(Branch.RegulatorTypes(rr),'Wye')
                     rrr=find(Branch.Wye3PhiBranchNumbers==l);
     ...
     if n~=Bus.SubstationNumber
        ...
        FromNeighborsNonRegSum+FromNeighborsRegSum-VnnTilde*conj(YCapTilde.')-SnTilde-ToNeighborsNonRegSum-ToNeighborsRegSum==0;

         PowerFlowConstraintsP((n-1)*3+1:3*n)=real(FromNeighborsNonRegSum+...
             FromNeighborsRegSum-VnnTilde*conj(YCapTilde.')-SnTilde-ToNeighborsNonRegSum-ToNeighborsRegSum==0 );
   
      
         PowerFlowConstraintsQ((n-1)*3+1:3*n)=imag(FromNeighborsNonRegSum+...
             FromNeighborsRegSum-VnnTilde*conj(YCapTilde.')-SnTilde-ToNeighborsNonRegSum-ToNeighborsRegSum==0);
     ...
     ThermalLossExpression=ThermalLossExpression+ sum(real(ToNeighborsNonRegSum+FromNeighborsNonRegSum+...
         ...
         FromNeighborsNonRegSum+FromNeighborsRegSum-VnnTilde*conj(YCapTilde.')+SIn-SnTilde-ToNeighborsNonRegSum-ToNeighborsRegSum==0;

         PowerFlowConstraintsP((n-1)*3+1:3*n)=real(FromNeighborsNonRegSum+...
             FromNeighborsRegSum-VnnTilde*conj(YCapTilde.')+SIn-SnTilde-ToNeighborsNonRegSum-ToNeighborsRegSum==0 );
